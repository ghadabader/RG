...
| 0003 | A user completes every chapter in the exam's current skill catalog (active time, excluding pauses) in under 3 hours; a chapter's score reflects the highest difficulty tier sustained, not raw percent-correct |
...
| 0005 | The history page renders an overall readiness-score trend chart across all past attempts, plus a per-chapter score comparison chart, so a user can see whether they improved — overall and per chapter — without opening each report individually |
