...
1. Switch UI language (AR / EN / HE) at any time via global chrome.
...
| 0006 | Switching to Arabic or Hebrew flips layout to `dir="rtl"` correctly across nav, forms, exam flow and report page |
...
| Must operate across three language interfaces: Arabic, English, Hebrew; Arabic and Hebrew are RTL | user | RTL is a real layout requirement, not just translated strings (requirement 0006) |
...
| Job-postings/course-catalog sources may have limited reach or relevance for Arabic/Hebrew-speaking markets if exam content stays English-only | Low–Medium | Low job-match relevance reported by AR/HE-locale users |

## 9. Open questions

1. Product name not yet chosen.
2. Does the AR/EN/HE requirement cover UI chrome only, or also exam content and the
   underlying job-posting/course-catalog sources? Affects whether a source like Adzuna has
   meaningful reach for the intended market.
3. Long-term monetization model (B2C freemium vs. B2B2C selling candidate signal to
   bootcamps/universities/recruiters) — deliberately deferred, not decided.
4. Free-tier LLM cost-control strategy — rate limits vs. eating cost as CAC — not decided.
5. Question bank sizing per (chapter, difficulty tier) needed so retakes don't feel
